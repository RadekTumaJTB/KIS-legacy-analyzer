/**
 * EditParticipationModal Component
 *
 * Modal for editing existing Equity Stake (Majetková Účast)
...
import { calculateAccountingAmounts, calculateTotalAmount } from '../../../types/asset';
...
interface EditParticipationModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSuccess: (updatedStake: EquityStake) => void;
  stake: EquityStake;
}
...
export const EditParticipationModal: React.FC<EditParticipationModalProps> = ({
  ...
  const [formData, setFormData] = useState<Partial<EquityStake>>(stake);
  ...
  // Reset form data when stake changes
  useEffect(() => {
    if (stake) {
      setFormData(stake);
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const result = await assetApi.updateEquityStake(stake.id, formData);
      onSuccess(result);
      handleClose();
    } catch (err) {
        ...
        <div className="flex items-center justify-between p-6 border-b sticky top-0 bg-white">
          <div>
            <h2 className="text-2xl font-bold text-gray-900">
              Upravit Majetkovou Účast
            </h2>
            <p className="text-sm text-gray-600 mt-1">ID: {stake.id}</p>
          </div>
          <button
            onClick={handleClose}
          ...
          <div className="p-6 space-y-6">
            ...
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div>
                  <span className="text-gray-600">Poslední úprava:</span>{' '}
                  <span className="font-medium text-gray-900">
                    {new Date(stake.lastModified).toLocaleString('cs-CZ')}
                  </span>
                </div>
                <div>
                  <span className="text-gray-600">Upravil:</span>{' '}
                  <span className="font-medium text-gray-900">{stake.modifiedByUser}</span>
                </div>
              </div>
            </div>
