...
//  }
  ...
  {
    if(!semaforPs) throw new KisException("V tomto okamžiku nelze spustit update protistran, protože je již spuštìn (od nìkoho jiného) a mohlo by dojít ke konfliktu... Pokud je to pøesto nutné, zkuste to za chvíli znovu.",
...
//      stmt.execute();
      ...
      logger.error("Reload protistran pro uživatele "+userName+" skonèil chybou!",s);
...
/*  
    ...
    try {
      ...
      st.execute();
      logger.info("Reload osob úspìšnì ukonèen!");
    }
    catch (SQLException s) {
      s.printStackTrace(); //pro zacatek
      logger.error("Reload osob skonèil chybou!",s);
