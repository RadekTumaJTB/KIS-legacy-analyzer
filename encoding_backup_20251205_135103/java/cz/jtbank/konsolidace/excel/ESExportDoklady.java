...
public class ESExportDoklady extends AbsExcelDoklad 
{
...
private Number   idDokladPodnikatelIfrs; /* esc IFRS*/
  ...
  public ESExportDoklady(ApplicationModule dokladyModule,
                         Number idDoklad,
                         String userMustek,
  ...
  /* esc IFRS podnikatelsky doklad*/
    dm.getTransaction().commit();

    vo = dm.findViewObject("KpDatDokladView1");
    vo.clearCache();
    vo.setWhereClause("ID_KTGUCETNISPOLECNOST = "+idKtgSpolecnost+" AND DT_DATUM=TO_DATE('"+sdf.format(datum)+"','dd.mm.yyyy') AND NL_KROK=20");
    if(vo.hasNext()) 
    {
      Row row = vo.next();
      idDokladPodnikatelIfrs = (Number) row.getAttribute("Id");
    }
    else 
    {
      logger.warn("Neexistuje podnikatelsk doklad IFRS pro "+idDoklad);
    }
    vo.closeRowSet();
    dm.getTransaction().commit();
...
*/    
    if ( mocVelky ) {
      ...
      setCellValue( listNr, ++rowNr, 0, "D A T A    N E J S O U   K O M P L E T N  - tento list obsahuje pouze nezaazen transakce" , null );
      setCellValue( listNr, ++rowNr, 0, "Detail je dostupn vhradn pes tlatko D u dokladu, vygenerovnm excelu s kompletnm detailem, pp. pmo vbrem SYU" , null );
    }
    vo.closeRowSet();
    dm.getTransaction().commit();
    //NEW
...
*/
    ...
    setCellValue( listNr, 2, 9, "Spolenost", null );
...
/*esc 03.05   podnikovy doklad bude aj v lokal aj v ifrs
    if(!baseDoklad) {
...
*/    
    ...
    outputZahlaviPodnikatel(nazevSpol,menaSpol,datum,listNr,jenomPs);
    ...
    if (flagIfrs)
        idDokladPodnikatel = idDokladPodnikatelIfrs;
    ...
    {      
      ...
      setCellValue(listNr+list-1,rowNr,colNr,koef*castkaLocal,style,false,"# ##0\\"); // format sa ingoruje, pouziva sa skopirovany style
...
*/
  ...
  public String getWarning() {
    ...
    if ( bilanceRozdil != 0 )
      ret.append( "Rozdl bilance=" + bilanceRozdil + "<br>" );
    if ( noDetail ) 
      ret.append( "dn etn zznam.<br>" );
    if ( sumaZiskZtrata != 0 )
      ret.append( "Nesouhlas zisk ztrta vsledovka/pasiva. Rozdl="+sumaZiskZtrata+"<br>" );
    if ( pocetNezarazenych != 0 )
      ret.append( "Nezaazench zznam="+pocetNezarazenych+"<br>" );
    if ( pocetTestICO != 0 )
      ret.append( "Zznam bez I="+pocetTestICO );
