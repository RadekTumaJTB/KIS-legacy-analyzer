...
public class ESExportPodnikatelUcty extends AbsExcelDoklad 
{
  static Logger logger = Logger.getLogger(ESExportPodnikatelUcty.class);
  ...
  public ESExportPodnikatelUcty(ApplicationModule dokladyModule)
  {
    dm = dokladyModule;
    init();
  }
  ...
  private void init() {
    logger.info("ExportPodnikatelUcty");  

    setFileName ( "BilancePodnikatelUcty.xlsx" );
    setFileRelativeName( Constants.DIR_PODNIKATEL_UCTY+"\\"+getFileName() );
  ...
  private void outputDetail()
  {
    ...
    setCellValue( listNr, rowNr, colNr, "ID Subjektu" , lightBlue ); colNr++;
    ...
    vo = dm.findViewObject("VwKpDokladautoPodnikatelView1");
    vo.clearCache();
    ...
    while ( vo.hasNext() && pocet-- > 0 ) {
      ...
      setCellValue( listNr, rowNr, colNr, ""+row.getAttribute("IdCissubject"), null );                        
      colNr++;
  ...
  public static void main(String[] argv) 
  {
    try {
      ...
      ESExportPodnikatelUcty ed = new ESExportPodnikatelUcty(dm);
      ed.excelOutput();
