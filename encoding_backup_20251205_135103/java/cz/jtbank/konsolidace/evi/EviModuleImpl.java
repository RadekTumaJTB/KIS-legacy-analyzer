...
//      if(hlp!=null) osoby[3] = hlp.intValue(); else osoby[3] = -1;
  ...
  public String getEmailText(int idUcSpol, String dbLink) 
  {
    ...
    StringBuffer text = new StringBuffer("V evidenci administrovaných spoleèností došlo ke zmìnì u spoleènosti '"+nazev+"' a bylo vyžádáno rozeslání tohoto informaèního e-mailu.");

    if(dbLink==null) return text.toString();
    
    text.append("\nVe \"vaší\" databázi "+dbLink+" jsou nìkteré údaje jinak...");
    ...
    while(voUS.hasNext()) 
    {
      ...
      if(!nazev.equals(sNazev)) 
        text.append("\nNázev spoleènosti v evidenci KIS je '"+nazev+"' zatímco u vás '"+sNazev+"'.");
      if(!equalsIco(ico,sIco)) 
        text.append("\nIÈ spoleènosti v evidenci KIS je '"+ico+"' zatímco u vás '"+sIco+"'.");
      if(!ulice.equals(sUlice)) 
        text.append("\nUlice sídla spoleènosti v evidenci KIS je '"+ulice+"' zatímco u vás '"+sUlice+"'.");
      if(!mesto.equals(sMesto)) 
        text.append("\nMìsto sídla spoleènosti v evidenci KIS je '"+mesto+"' zatímco u vás '"+sMesto+"'.");
      if(!psc.equals(sPsc)) 
        text.append("\nPSÈ sídla spoleènosti v evidenci KIS je '"+psc+"' zatímco u vás '"+sPsc+"'.");
      if(!stat.equals(sStat)) 
        text.append("\nStát sídla spoleènosti v evidenci KIS je '"+stat+"' zatímco u vás '"+sStat+"'.");
    }
    voUS.closeRowSet();
  ...
  public String getEmailNavrhText(int idSpol) 
  {
    ...
    StringBuffer text = new StringBuffer("V evidenci administrovaných spoleèností vznikl návrh zmìny u spoleènosti '"+nazev+"' a bylo vyžádáno rozeslání tohoto informaèního e-mailu.");

    if("1".equals(navrh))
      text.append("\nZmìna je navrhována v základních datech spoleènosti (název, IÈ...).");
    ...
    for(int i=0; i<8; i++) {
      switch (i) {
        ...
        case 1: vo = getKpDatEvizplnomocneniView1(); zmena="zplnomocnìní"; break;
        ...
        case 5: vo = getKpDatEvikomisionarskesmlouvyView1(); zmena="komisionáøské smlouvì"; break;
