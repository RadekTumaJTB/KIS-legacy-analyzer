...
public class AutoProtokolNew
{
  ...
  public boolean checkProtokol(ApplicationModule am) 
  {
    ...
    try {
      SystemStatus.getInstance().setStatus("Generovďż˝nďż˝ protokolďż˝");
...
System.gc();      
    try {
      SystemStatus.getInstance().setStatus("Kontrola schvalovďż˝kďż˝");
      checkSchvalovak();
...
System.gc();      
    try {
      SystemStatus.getInstance().setStatus("Kontrola budgetďż˝");
...
//System.gc();
    ...
    try {
      SystemStatus.getInstance().setStatus("Kontrola zprďż˝v karet projektďż˝");
      checkProjektMemorandum();
      logger.debug("PROTOKOL - Dokoncena kontrola informacnich zprav karet projektu");

      SystemStatus.getInstance().setStatus("Kontrola pďż˝ecenďż˝nďż˝ projektďż˝");
    ...
    try {
      SystemStatus.getInstance().setStatus("Kontrola zprďż˝v karet SPV");
    ...
    try {
      SystemStatus.getInstance().setStatus("Kontrola administrovanďż˝ch spoleďż˝nostďż˝");
    ...
    try {
      SystemStatus.getInstance().setStatus("Kontrola schvalovďż˝nďż˝ dokladďż˝");
      checkSpolecnostZamekGen();
      logger.debug("PROTOKOL - Excel se schvalenimi a zamitnutimi");
  
      SystemStatus.getInstance().setStatus("Kontrola spoleďż˝nostďż˝ kons. skupiny 4");
      checkSpolecnostSkupina4();
      logger.debug("PROTOKOL - Excel se spolecnostmi vs. skupina 4");
      
      SystemStatus.getInstance().setStatus("Kontrola souladu admin. a evid. spoleďż˝nostďż˝");
      checkSpolecnostiSoulad();
      logger.debug("PROTOKOL - Excel s nesouladem ve spolecnostech adm. vs. evi.");
    
      checkZamekVSClose();
      logger.debug("PROTOKOL - Nesoulad zamykďż˝nďż˝ a last close date");
    ...
    try {
      SystemStatus.getInstance().setStatus("Kontrola zmďż˝n v protistranďż˝ch v primďż˝rnďż˝ch systďż˝mech");
      checkZmenyProtistran();
      logger.debug("PROTOKOL - Excel se zmďż˝nami v protistranďż˝ch v primďż˝rnďż˝ch systďż˝mech");
    } 
    catch(Throwable t) 
    {
    ...
    try {
      SystemStatus.getInstance().setStatus("Kontrola chybďż˝jďż˝cďż˝ch nďż˝kladovďż˝ch ďż˝ďż˝tďż˝ v pďż˝evodovďż˝ch mďż˝stcďż˝ch budgetďż˝");
      checkBudgetMustekNaklad();
      logger.debug("PROTOKOL - Excel s chybďż˝jďż˝cďż˝mi nďż˝kladovďż˝mi ďż˝ďż˝ty v pďż˝evodovďż˝ch mďż˝stcďż˝ch budgetďż˝");
    } 
    catch(Throwable t) 
    {
  ...
  private boolean isEnabledEmailByDay() 
  {
	...
	if (logger.isDebugEnabled()){
		...
		logger.debug("am={}âam", am==null?"null":am.toString());
	}
    ViewObject vo = am.findViewObject("KpKalendarakciView1");
	if (logger.isDebugEnabled()){
		logger.debug("vo={}âvo", vo==null?"null":vo.toString());
	}	
    vo.clearCache();
  ...
  private String checkLastCloseDate(Number idSpol)
  {
    ...
    {
      return "Nenďż˝ nastaven termďż˝n uzavďż˝enďż˝ ďż˝ďż˝etnďż˝ho obdobďż˝!";
    }
    ...
    {
      return "Proďż˝lďż˝ termďż˝n uzavďż˝enďż˝ ďż˝ďż˝etnďż˝ho obdobďż˝! Naposledy bylo uzavďż˝eno ďż˝ďż˝etnďż˝ obdobďż˝ k "+sdf.format(lastCloseDate.dateValue());
    }
  ...
  private void checkZamekVSClose()
  {
    ...
    while(voSpol.hasNext()) 
    {
      ...
      set.add(nazev+" ("+id+") [v ďż˝ďż˝etnictvďż˝ "+dt+"] Zďż˝:"+zu+", Nďż˝:"+nu);
...
//                      " AND NL_KROK = 1 AND EXISTS (SELECT NULL FROM DB_JT.KP_KTG_DOKLAD KKD WHERE KKD.ID = KpDatDoklad.ID_KTGDOKLAD AND KKD.ID_CISDOKLAD = 1)" +
    ...
    while(vo.hasNext()) 
    {
      ...
      buf.append("Neschvďż˝lenďż˝ doklady "+Constants.kym[typeUser]+" pro termďż˝n "+sdf.format(dtDatum.dateValue())+"<br>");
    }
    vo.closeRowSet();
  ...
  private String checkZamitnuto(Number idSpol, int typeUser)
  {
    ...
    while(vo.hasNext()) 
    {
      ...
      buf.append("<font color='red'>Zamďż˝tnutďż˝ doklady "+Constants.kym[typeUser]+" pro termďż˝n "+sdf.format(dtDatum.dateValue())+": "+duvod+"</font><br>");
    }
    vo.closeRowSet();
...
//        voDoklad.setWhereClause( where + " AND NL_KROK = 1");
        ...
        if(voDoklad.hasNext()) 
        {
          ...
          //MAJETKOVE UCASTI
          String muk = "Majetkovďż˝ ďż˝ďż˝asti: ";
          try { 
            muk += ((DokladyModule)am).majetekUcastKontrola(idDoklad.intValue());
          } 
          catch (KisException e) {
            muk += "Chyba volďż˝nďż˝ procedury majetekUcastKontrola!";
          }
          //LAST CLOSE DATE
          ...
          //if(typeUser==Constants.UCETNI && schvalenoUcet.indexOf("Neschvďż˝lenďż˝ doklady "+Constants.kym[Constants.UCETNI])<0) addToLog = false;
...
//        voDoklad.setWhereClause( where + " AND NL_KROK = 1");
        ...
        if(voDoklad.hasNext()) 
        {
          ...
          oracle.jbo.domain.Date dtVytvoreno = (oracle.jbo.domain.Date) rowDoklad.getAttribute("DtVytvoreno");
          voProtokol.setWhereClause("ID_DOKLAD = "+idDoklad+
                                    " AND lower(S_HLASKA) like '%nenalezen unifikovanďż˝%'");
          //UNIF. UCTY
          StringBuffer buf = new StringBuffer();
          if(!voProtokol.hasNext()) 
          {
  ...
  {
    String ret = "Nebylo moďż˝no vytvoďż˝it doklad pro spoleďż˝nost "+nazevSpol+" ("+idSpol+").";
    ...
    if(voFronta.hasNext()) 
    {
      ...
      if(stav != null) 
      {
        if(stav.startsWith("Zam"))
          ret = "Zamďż˝enďż˝ doklad pro spoleďż˝nost "+nazevSpol+" ("+idSpol+") nenďż˝ moďż˝no pďż˝egenerovat.";
        else if(stav.startsWith("DB"))
          ret = "Databďż˝zovďż˝ chyba : "+sqlErr+" u spoleďż˝nosti "+nazevSpol+" ("+idSpol+").";
        else if(stav.startsWith("Exp"))
          ret = "Chyba pďż˝i exportu do excelu u spoleďż˝nosti "+nazevSpol+" ("+idSpol+").";
      }
    }
  ...
  {
    String ret = "<form><input type='checkbox' id='cb' checked onClick='javascript:switchCB()'>Zobrazit logy, kterďż˝ jsou v poďż˝ďż˝dku.\n";
    ret += "<input type='checkbox' id='rkc' checked onClick='javascript:switchRKC()'>Zobrazit i ne-RKC spoleďż˝nosti.\n";
    ret += "<input type='checkbox' id='error' checked onClick='javascript:switchError()'>Zobrazit i chybovďż˝ hlďż˝ky.</form>\n";
    ret += "Generovďż˝nďż˝ k "+sdf.format(datum)+"\n";
    ret += "<table class=\"clsTable\">\n"+
           "<tr class=\"clsTableRow\">\n"+
           "<th class=\"clsTableHeader\">Log</th>\n"+
           "<th class=\"clsTableHeader\">Dogenerovďż˝no v</th>\n"+
           "<th class=\"clsTableHeader\">Spoleďż˝nost</th>\n"+
           "<th class=\"clsTableHeader\">Vďż˝sledek</th>\n"+
           "<th class=\"clsTableHeader\">Id spol.</th>\n"+
           "<th class=\"clsTableHeader\">Detailnďż˝ log</th>\n"+
           "</tr>\n";
    protokol.append(ret);
  }
  ...
  private boolean isOk(String[] sa) 
  {
    ...
    {
      if(sa[i].indexOf("uzavďż˝enďż˝ ďż˝ďż˝etnďż˝ho obdobďż˝")>=0) return false;
      if(sa[i].indexOf("Neschvďż˝lenďż˝ doklad")>=0) return false;
    }
    return true;
  }
  ...
  private void genLogErrorRow(String errorMsg)
  {
    ...
    if(errorMsg.startsWith("Zam")) {
      ...
      protokol.append("<td>Problďż˝m</td>\n");  
    }
  ...
  System.gc();
    if(this.am == null) this.am = am;
   try {
      SystemStatus.getInstance().setStatus("manual- Kontrola budgetďż˝ Denni start ");
...
//12/2010      checkBudgetSchvalovani();
      SystemStatus.getInstance().setStatus("manual- Kontrola budgetďż˝ Denni konec");
...
System.gc();
    ...
    try {
      SystemStatus.getInstance().setStatus("manual - Kontrola zprďż˝v karet projektďż˝");
      checkProjektMemorandum();
    ...
    try {
      SystemStatus.getInstance().setStatus("manual - Kontrola zprďż˝v karet SPV");
      checkSpolecnostZprava();
