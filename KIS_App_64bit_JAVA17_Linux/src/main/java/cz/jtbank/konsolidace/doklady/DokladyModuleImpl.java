...
*/
  ...
  public void calculateDoklad(java.sql.Connection con) throws KisException
  {
    ...
    if(AutoMUCounter.getInstance().checkFinished(this)) {
      logger.info("calculateDoklad - Volani genMU");
      SystemStatus.getInstance().setStatus("Generování měnových a úvěrových pozic");
      genMU();
      MUProtokol.getInstance().createProtokol(this,-1);
      MUProtokol.getInstance().createProtokol(this,1);
      MUProtokol.getInstance().createProtokol(this,0);
    ...
    if((isDenni || isAutomat) && checkAutoGenFronta(0)==0 && checkAutoGenFronta(2)==0 && checkAutoGenFronta(99)==0) {
      ...
      SystemStatus.getInstance().setStatus("Denní spouštění na závěr aut. generování");
      denniSpousteni();
...
*/      
      ...
      SystemStatus.getInstance().setStatus("Automatické generování dokladů projektů");
      ulozDokladyProjekty();
      SystemStatus.getInstance().setStatus("Konec seriového generování (ještě běží paralelní procesy!)");
      */
    }
...
//SystemStatus.getInstance().setStatus("Závěr aut. generování( calculateDoklad)");    
  }
  ...
  private void reloadDataOLAP() //throws KisException
  {
    ...
    CallableStatement st = null;
    try {
      logger.info("reloadDataOLAP - reload dat pro OLAP - probíhá sériově");
      SystemStatus.getInstance().setStatus("Reload dat pro OLAP - probíhá sériově");
...
/*
    if(semaforIdSub == idSubkonsolidace) 
      throw new KisException("V tomto okamžiku nelze spustit generování subkonsolidačního dokladu pro subkonsolidaci "+idSubkonsolidace+", protože je již spuštěno (od někoho jiného) a mohlo by dojít ke konfliktu... Zkuste to za chvíli znovu.",
...
*/    
  ...
  public void vazbyManExcel(int idDoklad, int idSkup) throws KisException
  {
    ...
    try {
      ...
      */
      ...
      logger.error("Selhalo generování excelů vazby (man.)",e);
      throw new KisException("Selhalo generování excelů vazby (man.)",e);
    }
    finally 
    {
      try {
  ...
  private void existujeSubDoklad(int idSubkons, java.sql.Date datum) throws KisException
  {
    ...
    if(!maDoklad) throw new KisException("Neexistuje doklad pro sub-konsolidaci "+idSubkons+" pro datum "+sdf.format(datum)+
                                         "!!! Nejprve je třeba tento doklad vygenerovat, pak je teprve možno jej exportovat.", 
                                         new KisException(Constants.ERR_MESSAGE_ONLY));
  }
  ...
  public int checkAutoGenFronta(int jenomPs)
  {
    ...
    try {
      ...
      ResultSet rs = st.executeQuery("SELECT COUNT(*) AS CNT " +
                                     ...
                                     "WHERE (S_STAV = 'Čeká' OR S_STAV = 'Generuje se') " +
                                       "AND AUZIVATEL = '" + user + "' " +
  ...
  private void checkFrontaKonecGenerovani(String user)
  {
    ...
    if(cnt>0) {
      ...
      mail.sendMail("KIS",
                    ...
                    "KIS - problém při automat. generování",
                    "Při automatickém generování nastal v KISu problém. Ve frontě zbylo "+cnt+" žádostí o generování ve stavu 'DB chyba' od uživatele "+user+"!",
                    new String[] {});
    }
  }
...
*/
  ...
  {
    if(!semaforSub) throw new KisException("V tomto okamžiku nelze spustit generování subkonsolidačních dokladů, protože je již spuštěno (od někoho jiného) a mohlo by dojít ke konfliktu... Zkuste to za chvíli znovu.",
                                        new KisException(Constants.ERR_MESSAGE_ONLY));
    String userName = getUser();
    DBTransaction dbTran = this.getDBTransaction();
    CallableStatement st = null;
 ...
 /*    String userMustek = userMustek = (String)getParameterFromDoklad(idDoklad,"SUzivatelmustek"); */
  ...
  private void nocniGenerovaniSubkonsolidaci() {
    ...
    vo.closeRowSet();
    logger.info("Začátek generování subkonsolidací k "+datum);
    ...
    while(iter.hasNext()) 
    {
      ...
      {
        logger.error("Chyba při generování subkonsolidace "+id+" k "+datum,kex);
      }
    }
  ...
  public void exportCartesis() {
    ...
    {
      logger.error("Selhalo generování excelu směnných kurzů pro Cartesis",e);
    }
