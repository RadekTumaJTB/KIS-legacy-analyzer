  ...
  public void run() {
    ...
    for (;;) {
      ...
      try {
        ...
        SystemStatus.getInstance().setStatus("Generovďż˝nďż˝ spuďż˝tďż˝no");
        reloadProtistrany();
        generujAll();
//12.01.2010 zrusene MIS        generujMis();
generujDenni();
//12.01.2010 zrusene MIS       generujDenniMis();
        SystemStatus.getInstance().setStatus("Automatickďż˝ generovďż˝nďż˝ dokladďż˝");
      } 
...
*/
  ...
  /* 18.4.2007 - inovovana verze */
    SystemStatus.getInstance().setStatus("Vklďż˝dďż˝nďż˝ do fronty");
...
//      dm.deleteStareDoklady();
  ...
  private void generujDenni() {
	...
	//esc  pďż˝es tyden generovat JEN pro spolecnosti, kterďż˝ jsou RKC a majďż˝ budget
	  else {
