...
//
  ...
  protected void outputCartesisCode_S370() throws KisException, IOException
  {
    ...
    try {
	  ...
	  //
	  //esc novy select pre "Výsledok hospodárenia za účtovné obdobie bez menšinových podílů /+/-/   r. 001 - (r. 067 + r. 071 + r. 078 + r. 082 + r. 086 + r. 116)"
...
//
  ...
  protected void outputSegmentyLocal() throws KisException, IOException
  {
	...
	/*
	 * zustatek unifikovaneho uctu v lokalni méně s protistranou pro ucty, které jsou oznaceny jako CounterpartiesDetail 
...
/* archiv s tim koupeno		
			String sqlSelect = 
				...
				"where  " +
					...
					"f.s_kod = '" + flow + "' and " +
				   "kurzy.f_getMenaKurs ( 'ČNB', a.s_mena, 'SKK', d.dt_datum ) *  a.nd_castkalocal > uu.nd_limitcounterpartiesdetal and " +
...
//esc19.05 a  01.04.2008 bez sum > limit a podla spolocnosti 
				...
				"having " +
				"Abs(sum(aa.nd_castkalocal)) / kurzy.f_getMenaKurs ( 'ČNB', dd.s_mena, 'SKK', dd.dt_datum ) > uuu.nd_limitcounterpartiesdetal "	+
				" ) " +
...
//					+				"having " +				"Abs(sum(a.nd_castkalocal)) / kurzy.f_getMenaKurs ( 'ČNB', d.s_mena, 'SKK', d.dt_datum ) > uu.nd_limitcounterpartiesdetal "					
				;
...
/*
 *  zustatek unifikovaneho uctu v originalni méně ve vysi ekvivalentu v lokalni méně s protistranou pro ucty, 
...
//esc 19.05 a 01.04.2008
				" group by  " +
					...
					"d.id_ktgUcetniSpolecnost, d.dt_datum, d.s_mena " 
				;
//esc 19.05 rusim				+ "having " +				" Abs( sum(a.nd_castkalocal)) / kurzy.f_getMenaKurs ( 'ČNB', d.s_mena, 'SKK', d.dt_datum ) > uu.nd_limitcounterpartiesdetal "									
...
//esc 19.05					aa.S_MENA, 
					...
					"dd.id_ktgUcetniSpolecnost, dd.dt_datum, dd.s_mena " +
				"having " +
				" Abs( sum(aa.nd_castkalocal)) / kurzy.f_getMenaKurs ( 'ČNB', dd.s_mena, 'SKK', dd.dt_datum ) > uu.nd_limitcounterpartiesdetal "+
				" ) "+
//esc 01.04.2008				   			   
				"group by  " +
					"a.ID_DOKLAD, a.S_MENA, a.s_ucetunif, " +
...
/*10-2022*/
/*12-2022*/
/*Výkonné a nevýkonné expozice – I-RKC-019 – úvěry a pohledávky, které jsou předmětem znehodnocení*/
...
//SD #112241 
//19.03.2019 Do SAPu již není potřeba dotahovat rozpad významných protistran s transakční měnou z KISu – sekce LimitCounterPartiesMena.
...
//SD #109172
//02.01.2019 Do SAPu již není potřeba dotahovat rozpad IFRS typů s transakční měnou z KISu – sekce TypeMena.      
...
//SD #101283
//29.08 2018 Pro KIS to znamená, že již nepotřebujeme v exportním souboru package.csv řádky se STAT.A, STAT.L, STAT.E, STAT.R
