...
public class ESExportBudgetProjekt extends AbsExcelDoklad 
{
  static Logger logger = Logger.getLogger(ESExportBudgetProjekt.class);
  ...
  public ESExportBudgetProjekt(ApplicationModule dokladyModule,
                               java.sql.Date datum,
                               int userId,
                               int exportType,
                               boolean detail,
  ...
  private void init() {
    logger.info("ExportBudgetProjekt:userId="+userId+",exportType="+exportType);  

    setFileName ( (manual?"":"x")+"BudgetProjekt_Typ"+exportType+"_"+datum+".xlsx" );
    setFileRelativeName( Constants.DIR_BUDGET_PROJEKT+"\\"+userId+"\\"+getFileName() );
...
whereList.add("TO_DATE('"+sdf.format(datum)+"','dd.mm.yyyy') BETWEEN DT_PLATNOSTOD AND DT_PLATNOSTDO");
    if(exportType == ESExportBudgetStd.EXPORT_TYPE_OO_OJ) 
    {
      String wZastup = "(SELECT ID_KOHO FROM DB_JT.KP_DAT_BUDGETZASTUP WHERE ID_KDO = "+userId+" AND (DT_PLATNOSTOD IS NULL OR DT_PLATNOSTOD<=TO_DATE('"+sdf.format(datum)+"','dd.mm.yyyy')) AND (DT_PLATNOSTDO IS NULL OR DT_PLATNOSTDO>=TO_DATE('"+sdf.format(datum)+"','dd.mm.yyyy')) AND (VwDatBudgetprojekt.ID = ID_BUDGET OR ID_BUDGET IS NULL))";
/*
...
*/
whereList.clear();
whereList.add("(ID_PMANAGER = "+userId+" OR ID_UCSPOLOO = "+userId+" OR ID_PROJEKTTOP = "+userId+") AND TO_DATE('"+sdf.format(datum)+"','dd.mm.yyyy') BETWEEN DT_PLATNOSTOD AND DT_PLATNOSTDO");
whereList.add("(ID_PMANAGER IN "+wZastup+" OR ID_UCSPOLOO IN "+wZastup+" OR ID_PROJEKTTOP IN "+wZastup+") AND TO_DATE('"+sdf.format(datum)+"','dd.mm.yyyy') BETWEEN DT_PLATNOSTOD AND DT_PLATNOSTDO");
    }
    else if(exportType == ESExportBudgetStd.EXPORT_TYPE_VIEW_SPOL) 
    {
/*    
...
whereList.add("EXISTS (SELECT NULL "+
                        "FROM DB_JT.KP_REL_APPUSER_PROJEKT r "+
                        "WHERE VwDatBudgetprojekt.ID_KTGPROJEKT = r.ID_KTGPROJEKT "+
                          "AND r.ID_APPUSER = "+userId+") AND TO_DATE('"+sdf.format(datum)+"','dd.mm.yyyy') BETWEEN DT_PLATNOSTOD AND DT_PLATNOSTDO");
    }
    else if(exportType == ESExportBudgetStd.EXPORT_TYPE_VIEW_SPOL_ALL) 
    {
whereList.clear();
whereList.add("TO_DATE('"+sdf.format(datum)+"','dd.mm.yyyy') BETWEEN DT_PLATNOSTOD AND DT_PLATNOSTDO");
    }
...
//      setCellValue( listNr, rowNr, colNr, nazevGroup , styleBold );                        
    ...
    int celkemSloupcu = 0;
    ViewObject voBud = dm.findViewObject("VwDatBudgetprojektView1");
    ...
    voBunka.clearCache();
    ViewObject voBunkaData1 = dm.findViewObject("VwDatBudgetProjektpolozkadata1PARView1");
    voBunkaData1.clearCache();
    ViewObject voBunkaData2 = dm.findViewObject("VwDatBudgetProjektpolozkadata2PARView1");
...
boolean first = true;
    ...
    while(iterCnt.hasNext()) 
    {
      ...
      voBud.clearCache();
      voBud.setWhereClause(whereOJ);
      voBud.setOrderByClause("S_NAZEV");
      while(voBud.hasNext()) 
      {
        ...
        String kod = rowBud.getAttribute("IdKtgprojekt").toString();
        ...
        while(iGroup.hasNext()) 
        {
          ...
          while ( iter.hasNext() ) {
            ...
            voBunkaData1.setWhereClauseParam(1,new Integer(idBud));
            voBunkaData1.setWhereClauseParam(2,new Integer(idTran));
            ...
            voBunkaData2.setWhereClauseParam(1,new Integer(idBud));
            voBunkaData2.setWhereClauseParam(2,new Integer(idTran));
  ...
  private void outputListyGestor()
  {
    ...
    ViewObject vo = dm.findViewObject("VwDatBudgetprojektView1");
    String whereTT = "C_DEVELOPER = '0' AND C_SPECIAL='0' AND TO_DATE('"+sdf.format(datum)+"','dd.mm.yyyy') BETWEEN DT_PLATNOSTOD AND DT_PLATNOSTDO";
    String wZastup = "(SELECT ID_KOHO FROM DB_JT.KP_DAT_BUDGETZASTUP WHERE ID_KDO = "+userId+" AND (DT_PLATNOSTOD IS NULL OR DT_PLATNOSTOD<=TO_DATE('"+sdf.format(datum)+"','dd.mm.yyyy')) AND (DT_PLATNOSTDO IS NULL OR DT_PLATNOSTDO>=TO_DATE('"+sdf.format(datum)+"','dd.mm.yyyy')) AND ID_BUDGET IS NULL)";
    if(exportType != ESExportBudgetStd.EXPORT_TYPE_GESTOR_ALL) {
      whereTT += " AND (ID_GESTOR_CZ = "+userId+
    ...
    while ( vo.hasNext() ) {
      ...
      String kod = (String) row.getAttribute("IdKtgprojekt");
  ...
  private void outputDetail()
  {
    ...
    if(voDetail==null) {
      voDetail = dm.findViewObject("VwDatBudgetprojekttransakceView1");
      voDetail.clearCache();
    }
  ...
  protected boolean outputData () 
  {
    ...
    if(exportType != ESExportBudgetStd.EXPORT_TYPE_GESTOR && exportType != ESExportBudgetStd.EXPORT_TYPE_GESTOR_ALL) {
      outputListyStd();
    }
    else {
      outputListyGestor();
  ...
  public static void main(String[] argv) 
  {
    try {
      ...
      ESExportBudgetProjekt ed = new ESExportBudgetProjekt(dm,
                                                   new java.sql.Date(System.currentTimeMillis()),
                                                   24,
                                                   ESExportBudgetStd.EXPORT_TYPE_KONTROLING,
                                                   false,
                                                   true);
/*                                                   
      ESExportBudgetProjekt ed = new ESExportBudgetProjekt(dm,
                                                   new java.sql.Date(106,2,8),
                                                   //new java.sql.Date(System.currentTimeMillis()),
                                                   63,
