...
public class ESExportKamil extends AbsExcelDoklad 
{
  static Logger logger = Logger.getLogger(ESExportKamil.class);
  ...
  public ESExportKamil(ApplicationModule dokladyModule,
                       String idDoklad,
                       String locale,
                       int typRozkladu)
  {
  ...
  private void init() {
    ...
    logger.info("ExportKamil:nazevSpol="+nazevSpol+",idDoklad="+idDoklad);  

    setFileName ( "KamilPrehled_"+idDoklad+"_"+typRozkladu+".xlsx" );
  ...
  private void init2() {
    ...
    logger.info("ExportKamil:nazevSpol="+nazevSpol+",idSub="+idSub);  
    
    setFileName ( "KamilPrehled_"+idSub+"_"+typRozkladu+".xlsx" );
    setFileRelativeName( Constants.DIR_DOKLAD_KAMIL+"\\"+getFileName() );
    setFileAbsoluteName( OUT_DIR + this.getFileRelativeName() );
    setSablona( Constants.SABLONY_FILES_PATH+"Empty.xlsx" );
  }
  ...
  private void outputRozklad()
  {
    ...
    setCellValue( listNr, rowNr, 0, "BILANCE" , lightGray );
    setCellValue( listNr, rowNr, 1, "" , lightGray );
    
    ViewObject vo = dm.findViewObject("KpCisRadektextlangKamilView1");
    vo.clearCache();
    ...
    while ( vo.hasNext() ) {
      ...
      String wherePrehled = "ID_DOKLAD = "+idDoklad;
...
/*
      ESExportKamil ed = new ESExportKamil(dm,
                                               "246177",
                                               "cs_CZ",
                                               0);
      ed.excelOutput();
