...
public class ESExportDoklady2011_bck extends AbsExcelDoklad 
{
  ...
  private void init() {
    ...
    {
      logger.warn("Neexistuje podnikatelskďż˝ doklad pro "+idDoklad);
    }
    vo.closeRowSet();
    dm.getTransaction().commit();
...
//    }
    ...
    while ( vo.hasNext() && pocet-- > 0 ) {
      ...
      colNr++;
      if(clenJTFG) setCellValue(listNr,rowNr,colNr,"ďż˝len JTFG kons.",null);
      colNr++;
...
*/    
    if ( mocVelky ) {
      ...
      setCellValue( listNr, ++rowNr, 0, "D A T A    N E J S O U   K O M P L E T N ďż˝ - tento list obsahuje pouze nezaďż˝azenďż˝ transakce" , null );
      setCellValue( listNr, ++rowNr, 0, "Detail je dostupnďż˝ vďż˝hradnďż˝ pďż˝es tlaďż˝ďż˝tko D u dokladu, vygenerovďż˝nďż˝m excelu s kompletnďż˝m detailem, pďż˝ďż˝p. pďż˝ďż˝mo vďż˝bďż˝rem SYUďż˝" , null );
    }
    vo.closeRowSet();
    dm.getTransaction().commit();
    //NEW
...
*/
    ...
    while(iterGroup.hasNext()) 
    {
      ...
      while(iterProti.hasNext()) 
      {
        ...
        if(!lastProti.notProti) {
          ...
          setCellValue( listNr, rowNr, 1, "Celkem spoleďż˝nost " + lastProti.nazev, styleSpol );
    ...
    setCellValue( listNr, 2, 9, "Spoleďż˝nost", null );
  ...
  private void outputMenovaPozice() {
    ...
    for ( Iterator i = map.keySet().iterator(); i.hasNext(); ) {
      ...
      rowNr++;
      int colNr = 0;
      setCellValue( listNr, rowNr, colNr, "Dlouhďż˝ pozice", null );
      colNr++;
      ...
      rowNr++;
      colNr = 0;
      setCellValue( listNr, rowNr, colNr, "Krďż˝tkďż˝ pozice", null );
      colNr++;
  ...
  private void outputProtistrany() 
  {
    ...
    String ppf_info = (String) row.getAttribute("SAccPpfInfo");
      ...
      colNr++;
      if(clenJTFG) setCellValue(listNr,rowNrL1,colNr,"ďż˝len JTFG kons.",null);
      colNr++;
...
//esc 12/2009
  ...
  private void outputBilanceProtistrana() 
  {
    ...
    while(vo.hasNext()) 
    {
      ...
      colNr++;
      if(!opravna && clenJTFG) setCellValue(listNr,rowNr,colNr,"ďż˝len JTFG kons.",null);
  ...
  private void outputMPOP() 
  {
    ...
    while(vo.hasNext() && pocet-- > 0 ) 
    {
      ...
      colNr++;
      if(clenJTFG) setCellValue(listNr,rowNr,colNr,"ďż˝len JTFG kons.",style);
      colNr++;
...
*/
    ...
    if (vo.hasNext()) {
      ...
      setCellValue( listNr, rowNr, 1, "D A T A    N E J S O U   K O M P L E T N ďż˝ - poďż˝et ďż˝ďż˝dkďż˝ pďż˝esahuje moďż˝nosti excelu" , null );
    }
    vo.closeRowSet();
    dm.getTransaction().commit();
...
*/
  ...
  public String getWarning() {
    ...
    if ( bilanceRozdil != 0 )
      ret.append( "Rozdďż˝l bilance=" + bilanceRozdil + "<br>" );
    if ( noDetail ) 
      ret.append( "ďż˝ďż˝dnďż˝ ďż˝ďż˝etnďż˝ zďż˝znam.<br>" );
    if ( sumaZiskZtrata != 0 )
      ret.append( "Nesouhlasďż˝ zisk ztrďż˝ta vďż˝sledovka/pasiva. Rozdďż˝l="+sumaZiskZtrata+"<br>" );
    if ( pocetNezarazenych != 0 )
      ret.append( "Nezaďż˝azenďż˝ch zďż˝znamďż˝="+pocetNezarazenych+"<br>" );
    if ( pocetTestICO != 0 )
      ret.append( "Zďż˝znamďż˝ bez Iďż˝="+pocetTestICO );
