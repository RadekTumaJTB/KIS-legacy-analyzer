...
public class ESExportProjektCashFlowAll extends AbsExcelDoklad 
{
  static Logger logger = Logger.getLogger(ESExportProjektCashFlowAll.class);
  ...
  public ESExportProjektCashFlowAll(ApplicationModule dokladyModule,
                                 java.sql.Date datum)
  {
    logger.info("ESExportProjektCashFlowAll:datum="+datum);  
    dm = dokladyModule;
  ...
  private void init() {
    setFileName ( "ProjektyCF_"+datum+".xlsx" );
  ...
  private void outputProjekty() {
    ...
    ViewObject vo = dm.findViewObject("KpDatProjektcashflowExportView1");
    vo.clearCache();
    ...
    while(vo.hasNext()) 
    {
      ...
      projekt = row.getAttribute("SNazev")==null?"":(String)row.getAttribute("SNazev");
      ...
      colNr++;
      setCellValue( listNr, rowNr, colNr, projekt , null ); 
      colNr++;
  ...
  protected boolean outputData () 
  {
      ...
      logger.debug("projektyCFAll:"+((end-start)/1000.0)+"s");
  ...
  public static void main(String[] argv) 
  {
    try {
      ...
      ESExportProjektCashFlowAll ed = new ESExportProjektCashFlowAll(dm,
