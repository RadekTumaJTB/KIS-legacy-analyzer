...
public class ESExportSLPostup extends AbsExcelDoklad 
{
  static Logger logger = Logger.getLogger(ESExportSLPostup.class);
  ...
  public ESExportSLPostup(ApplicationModule dokladyModule,
                       java.sql.Date datum)
  {
    logger.info("ESExportSLPostup:datum="+datum);  
    dm = dokladyModule;
    this.datum = datum;
  ...
  private void init() {
    setFileName ( "SLPostup_"+datum+".xlsx" );
    setFileRelativeName( dir+"\\"+getFileName() );
    setFileAbsoluteName( OUT_DIR + this.getFileRelativeName() );
    setSablona( Constants.SABLONY_FILES_PATH+"Empty.xlsx" );
logger.info("ESExportSLPostup:cesta="+this.getFileAbsoluteName());  	
  }
  
  private void outputSLPostup() {
    ...
    rowNr++;
        ...
        ViewObject vo;
        vo = dm.findViewObject("VwRepSchvalovakpostupView1");
        vo.clearCache();
