...
public class ESExportDokladyNew extends AbsExcelDoklad 
{
  static Logger logger = Logger.getLogger(ESExportDokladyNew.class);
  static { logger.addAppender(Logging.getAppender(Logging.LOG_EXPORT_DOKLADY)); }

  private ApplicationModule dm;
  private Number idDoklad;
  private Number idDokladIFRS;
  private Number idDokladLocal;
  ...
  public ESExportDokladyNew(ApplicationModule dokladyModule,
                         Number idDokladLocal,
						 Number idDokladIFRS,
                         String userMustek,
  ...
  {
    dm = dokladyModule;
	this.idDokladLocal = idDokladLocal;
	if ( idDokladIFRS == null ) {
		idDokladIFRS = idDokladLocal;
	}
	this.idDoklad = idDokladIFRS;
    this.userMustek = userMustek;
    this.idDokladKamil = idDokladKamil;
	this.idDokladIFRS = idDokladIFRS;
  ...
  private void init() {
    ...
    String sablona = (localeSpol==null || localeSpol.length()<1) ? 
                     "SablonaBilance_NEW"+(sCNB?"CNB":"")+"_cs_CZ.xlsx" :
                     "SablonaBilance_NEW"+(sCNB?"CNB":"")+"_"+localeSpol+".xlsx";
...
System.out.println ( Constants.SABLONY_FILES_PATH+sablona );    
  ...
  private void outputHeaders() 
  {
    ...
    outputZahlaviNew(nazevSpol,menaSpol,datum,special,jenomPs);
/*    
...
*/
  ...
  private void outputPodnikatel() 
  {
    ...
    while(vo.hasNext()) 
    {
      ...
      if(list>1) list=2;//TESTOVACI RESENI - POZDEJI ODSTRANIT
      rowNr = radek + 3;
...
//    ret += flagPs ? "s PS" : "";
  ...
  public static void main(String[] argv) 
  {
    try {
      ...
      ESExportDokladyNew ed = new ESExportDokladyNew(dm,
                                               new Number(1864916),
											   new Number(1864919),
                                               null,
                                               null,
